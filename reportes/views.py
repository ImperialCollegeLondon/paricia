...
class CalidadConsultasPeriodo(PermissionRequiredMixin, FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.request.is_ajax():
            graf1 = getGrafico(
                estacion, variable, inicio, fin, frecuencia, profundidad=profundidad
            )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if not self.request.is_ajax():
                ...
                return render(
                    request, self.template_name, {"form": form, "mensaje": mensaje}
                )
        return render(
            request,
            self.template_name,
