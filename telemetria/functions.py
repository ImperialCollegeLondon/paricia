...
from telemetria.models import ConfigVisualizar, AlarmaEstado, TeleVariables, PrecipitacionAcumulada, \
    PrecipitacionEventos, PrecipitacionMultianual, ConfigCalidad
...
def consulta(estacion_id, inicio):
    ...
    for v in variables:
        ...
        sql = 'SELECT * FROM ' + tabla + ' WHERE estacion_id = %s AND fecha>= %s AND fecha<= %s AND valor IS NOT NULL ' \
                                         'order by fecha ASC;'
...
def consulta_calidad(estacion_id, inicio, usuario):
    ...
    for v in variables:
        ...
        sql2 = 'SELECT DISTINCT(profundidad) FROM ' + tabla + ' WHERE estacion_id = %s order by profundidad ASC;'
        ...
        for p in profundidades:
            n = p[0]
            _niveles = {'valor':[], 'fecha':[], 'fecha_anterior':None, 'umbral_superior':None, 'umbral_inferior':None}
            niveles[n] = objdict(_niveles)
            umbral_superior = config.get(variable_id=v.var_id, profundidad=n).umbral_superior
            niveles[n].umbral_superior = float(umbral_superior) if umbral_superior else None
            umbral_inferior = config.get(variable_id=v.var_id, profundidad=n).umbral_inferior
            niveles[n].umbral_inferior = float(umbral_inferior) if umbral_inferior else None
        frec_list = frecuencias.filter(var_id=v).order_by('fre_fecha_ini').values_list('fre_fecha_ini', 'fre_valor', named=True)
        ...
        for fila in consulta:
            ...
            if diff_dt.seconds > (frec_segundos + 40) or diff_dt.days > 0:
                ...
                niveles[fila.profundidad].fecha.append(
                    niveles[fila.profundidad].fecha_anterior + datetime.timedelta(minutes=int(frec_segundos/60))
                )
            niveles[fila.profundidad].valor.append(fila.valor)
            niveles[fila.profundidad].fecha.append(fila.fecha)
            niveles[fila.profundidad].fecha_anterior = fila.fecha
