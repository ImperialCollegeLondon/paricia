...
def generar_reportes_1variable(variable_id):
    ...
    es_reporte_automatico = Variable.objects.filter(
        var_id=variable_id, reporte_automatico=True
    )
...
def validacion_enviar(request):
    ...
    with connection.cursor() as cursor:
        cursor.callproc(
            "insertar_" + str(variable_id) + "validacion", [estacion_id, cambios_json]
        )
...
class ValidacionList(PermissionRequiredMixin, ListView, FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        context.update(
            pagination(self.object_list, context["page_obj"].number, self.paginate_by)
        )
...
class BorrarValidados(PermissionRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        filas_validado, filas_horario, filas_diario, filas_mensual = borrar_validados(
            estacion_id, var_id, inicio, fin
        )
...
class CalidadValidar(PermissionRequiredMixin, FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        tabla = reporte_validacion_profundidad(
            variable.var_id, estacion.est_id, profundidad, inicio, fin
        )
        _grafico = grafico(tabla, variable, estacion)

        return render(
            request,
            "validacion/calidad_validar_tabla.html",
            {
                "tabla": tabla,
                "variable": variable,
                "estacion": estacion,
...
class CalidadPeriodosValidacion(PermissionRequiredMixin, FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            return HttpResponse("")
        intervalos = periodos_validacion_profundidad(
            est_id=estacion.est_id, var_id=variable.var_id, profundidad=profundidad
        )
        return render(request, self.template_name, {"intervalos": intervalos})
...
class CalidadBorrarDatos(PermissionRequiredMixin, FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if "0" in tipo:
            crudos = borrar_crudos_profundidad(
                estacion, profundidad, variable, inicio, fin
            )
        if "1" in tipo:
            validados, horarios, diarios, mensuales = borrar_validados_profundidad(
                estacion, profundidad, variable, inicio, fin
            )
