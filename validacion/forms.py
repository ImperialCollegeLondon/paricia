...
from django.forms import ModelForm, Form, ModelChoiceField, DateTimeField, DateField, ChoiceField,\
...
class CalidadBorrarDatosForm(Form):
    ...
    variable = ModelChoiceField(queryset=Variable.objects.filter(pk__in=var_ids), label="Variable")
...
class ValidacionForm(Form):
    estacion = ModelChoiceField(queryset=Estacion.objects.order_by('est_id').all(), required=True, label="Estación")
    variable = ModelChoiceField(queryset=Variable.objects.order_by('var_id').all(), required=True)
    inicio = DateField(input_formats=['%Y-%m-%d'], label="Inicio (yyyy-mm-dd)", required=True)
    fin = DateField(input_formats=['%Y-%m-%d'], label="Fin (yyyy-mm-dd)", required=True)


class ValidacionCalidadForm(Form):
    estacion = ModelChoiceField(queryset=Estacion.objects.none(), required=True, label="Estación")
    var_ids = (101, 102, 103, 104, 105, 106, 107)
    variable = ModelChoiceField(queryset=Variable.objects.filter(pk__in=var_ids), required=True, label="Variable")
    profundidad = ChoiceField(choices=((None, ""),), required=True, label="Profundidad")
    inicio = DateField(input_formats=['%Y-%m-%d'], required=True, label="Inicio (yyyy-mm-dd)")
