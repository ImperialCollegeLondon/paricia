...
def periodos_validacion_profundidad(est_id, var_id, profundidad):
    ...
    SELECT * FROM reporte WHERE fecha_fin IS NOT NULL;
    """
    sql = sql.replace("%%var_id%%", str(var_id)).replace("%%est_id%%", str(est_id)).replace("%%profundidad%%", str(profundidad))
    result = consulta_validados.objects.raw(sql)
    return result
...
def reporte_validacion(form):
    ...
    inicio = datetime.datetime.combine(form.cleaned_data['inicio'], datetime.time(0, 0, 0, 0))
...
def reporte_validacion_profundidad(variable_id, estacion_id, profundidad, inicio, fin):
    ...
    query = "select * FROM reporte_validacion_profundidad_var" + str(variable_id) + "(%s, %s, %s, %s);"
    consulta = ReporteValidacion.objects.raw(query, [estacion_id, profundidad, inicio, fin])
    return consulta
...
def profundidades(estacion_id, variable_id):
    ...
    sql = "SELECT DISTINCT(profundidad) FROM medicion_var" + str(variable_id) + "medicion "
