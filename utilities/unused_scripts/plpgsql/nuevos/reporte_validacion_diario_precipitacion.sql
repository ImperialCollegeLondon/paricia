-- FUNCTION: public.reporte_validacion_diario_precipitacion(integer, timestamp with time zone, timestamp with time zone, numeric, numeric)

-- DROP FUNCTION public.reporte_validacion_diario_precipitacion(integer, timestamp with time zone, timestamp with time zone, numeric, numeric);
...
CREATE OR REPLACE FUNCTION public.reporte_validacion_diario_precipitacion(
	...
	_var_maximo numeric,
	_var_minimo numeric)
    RETURNS TABLE(id bigint, fecha timestamp with time zone, fecha_error numeric, fecha_numero numeric, valor numeric, porcentaje numeric, porcentaje_error boolean, valor_numero numeric, valor_error boolean, media_historica numeric, estado boolean, validado boolean, c_varia_err numeric) 
    LANGUAGE 'plpgsql'
