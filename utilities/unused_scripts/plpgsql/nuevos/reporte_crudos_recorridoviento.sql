-- FUNCTION: public.reporte_crudos_recorridoviento(integer, timestamp with time zone, timestamp with time zone, numeric, numeric)

-- DROP FUNCTION public.reporte_crudos_recorridoviento(integer, timestamp with time zone, timestamp with time zone, numeric, numeric);

CREATE OR REPLACE FUNCTION public.reporte_crudos_recorridoviento(
...
BEGIN
    ...
    validacion AS (
        ...
        FROM validacion_var16validado v WHERE v.estacion_id = (SELECT est_id FROM estacion)
    ...
    medicion AS (
        ...
        FROM medicion_var16medicion m WHERE m.estacion_id = (SELECT est_id FROM estacion)
...
-- ALTER FUNCTION public.reporte_crudos_recorridoviento(integer, timestamp with time zone, timestamp with time zone, numeric, numeric)
--    OWNER TO usuario1;
