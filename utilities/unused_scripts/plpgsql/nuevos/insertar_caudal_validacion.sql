-- FUNCTION: public.insertar_caudal_validacion(bigint, json)

-- DROP FUNCTION public.insertar_caudal_validacion(bigint, json);

CREATE OR REPLACE FUNCTION public.insertar_caudal_validacion(
	_estacion_id bigint,
	_datos json)
    RETURNS boolean
    LANGUAGE 'plpgsql'
...
DECLARE
	...
	id_insertado       validacion_var10validado.id%TYPE;
	valor numeric;
	maximo  numeric;
	minimo numeric;
BEGIN
    FOR row_datos in curs_datos LOOP
        ...
        IF row_datos.seleccionado is TRUE THEN
            INSERT INTO validacion_var10validado(estacion_id, fecha, valor, maximo, minimo, usado_para_horario, validacion)
            ...
            IF row_datos.comentario IS NOT NULL THEN
                INSERT INTO validacion_comentariovalidacion(variable_id, estacion_id, validado_id, comentario)
                    VALUES (10, _estacion_id, id_insertado, row_datos.comentario);
            END IF;
        END IF;
...
-- ALTER FUNCTION public.insertar_caudal_validacion(bigint, json)
--    OWNER TO usuario1;
