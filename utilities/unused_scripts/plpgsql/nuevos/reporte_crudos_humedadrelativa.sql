-- FUNCTION: public.reporte_crudos_humedadrelativa(integer, timestamp with time zone, timestamp with time zone, numeric, numeric)

-- DROP FUNCTION public.reporte_crudos_humedadrelativa(integer, timestamp with time zone, timestamp with time zone, numeric, numeric);

CREATE OR REPLACE FUNCTION public.reporte_crudos_humedadrelativa(
...
BEGIN
    ...
    validacion AS (
        ...
        FROM validacion_var3validado v WHERE v.estacion_id = (SELECT est_id FROM estacion)
    ...
    medicion AS (
        ...
        FROM medicion_var3medicion m WHERE m.estacion_id = (SELECT est_id FROM estacion)
...
-- ALTER FUNCTION public.reporte_crudos_humedadrelativa(integer, timestamp with time zone, timestamp with time zone, numeric, numeric)
--    OWNER TO usuario1;
