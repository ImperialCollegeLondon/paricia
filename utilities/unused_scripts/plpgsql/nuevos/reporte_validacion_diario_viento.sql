-- FUNCTION: public.reporte_validacion_diario_viento(integer, timestamp with time zone, timestamp with time zone, numeric, numeric)

-- DROP FUNCTION public.reporte_validacion_diario_viento(integer, timestamp with time zone, timestamp with time zone, numeric, numeric);
...
CREATE OR REPLACE FUNCTION public.reporte_validacion_diario_viento(
	...
	_var_maximo numeric,
	_var_minimo numeric)
    RETURNS TABLE(id bigint, fecha timestamp with time zone, fecha_error numeric, fecha_numero numeric, valor numeric, maximo numeric, minimo numeric, direccion numeric, categoria numeric, porcentaje numeric, porcentaje_error boolean, valor_error boolean, maximo_error boolean, minimo_error boolean, valor_numero numeric, maximo_numero numeric, minimo_numero numeric, estado boolean, validado boolean) 
    LANGUAGE 'plpgsql'
