-- FUNCTION: public.reporte_validacion_diario_temperaturaambiente(integer, timestamp with time zone, timestamp with time zone, numeric, numeric)

-- DROP FUNCTION public.reporte_validacion_diario_temperaturaambiente(integer, timestamp with time zone, timestamp with time zone, numeric, numeric);

CREATE OR REPLACE FUNCTION public.reporte_validacion_diario_temperaturaambiente(
...
BEGIN
    ...
    validacion AS (
        ...
        FROM validacion_var2validado v WHERE v.estacion_id = (SELECT est_id FROM estacion) AND v.fecha >= _fecha_inicio AND v.fecha <= _fecha_fin
    ...
    medicion AS (
        ...
        FROM medicion_var2medicion m WHERE m.estacion_id = (SELECT est_id FROM estacion) AND m.fecha >= _fecha_inicio AND m.fecha <= _fecha_fin
    ),
...
-- ALTER FUNCTION public.reporte_validacion_diario_temperaturaambiente(integer, timestamp with time zone, timestamp with time zone, numeric, numeric)
--    OWNER TO usuario1;
