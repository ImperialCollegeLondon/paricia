-- FUNCTION: public.reporte_validacion_agua(integer, timestamp with time zone, timestamp with time zone, numeric, numeric)

-- DROP FUNCTION public.reporte_validacion_agua(integer, timestamp with time zone, timestamp with time zone, numeric, numeric);
...
CREATE OR REPLACE FUNCTION public.reporte_validacion_agua(
	...
	_var_maximo numeric,
	_var_minimo numeric)
    RETURNS TABLE(id bigint, fecha timestamp with time zone, nivel numeric, caudal numeric, validado boolean, seleccionado boolean, estado boolean, fecha_error numeric, nivel_error boolean, caudal_error boolean, stddev_error boolean, comentario character varying, variacion_consecutiva numeric) 
    LANGUAGE 'plpgsql'
