...
def datos_grafico1(estacion_id, variables_id, profundidad, inicio, fin):
    ...
    for v in variables:
        ...
        sql = 'SELECT * FROM ' + tabla + ' WHERE estacion_id = %s AND profundidad = %s AND fecha>= %s AND fecha<= %s order by fecha ASC;'
...
def datos_grafico2(estacion_id, variables_id, profundidad, inicio, fin):
    ...
    for v in variables:
        tabla = 'validacion_var' + str(v.var_id) + 'validado'
        sql = 'SELECT fecha, valor FROM ' + tabla + ' WHERE estacion_id = %s AND profundidad = %s AND fecha>= %s AND fecha<= %s order by fecha ASC;'
        tabla = pd.DataFrame()
...
def datos_comparar_hidro(fre, _inicio, _fin, est_calidad, profundidad, var_calidad, est_hidro, var_hidro):
    ...
    for v in var_calidad:
        ...
        tabla = _model[fre.nombre].app + '_var' + str(v) + _model[fre.nombre].model.lower()
        sql = 'SELECT * FROM ' + tabla + ' WHERE estacion_id = %s AND profundidad = %s '
        sql += 'AND fecha>= %s AND fecha<= %s  AND valor IS NOT NULL ORDER BY fecha ASC;'
        consulta = Datos.objects.raw(sql, [est_calidad.est_id, profundidad, inicio, fin])
        ...
        frecuencias = freq_calidad.filter(var_id=v).order_by('fre_fecha_ini').values_list('fre_fecha_ini', 'fre_valor', named=True)
        frecuencias = construir_intervalos_freq(frecuencias, inicio_datetime, fin_datetime)
        ...
        for fila in consulta:
            ...
            diff_dt = fila.fecha - fecha_anterior
            while not (intervalo_frecuencia['inicio'] <= fila.fecha < intervalo_frecuencia['fin']):
                intervalo_frecuencia = frecuencias.pop()
                frec_segundos = intervalo_frecuencia['freq'] * 60

            if diff_dt.seconds > (frec_segundos + 40) or diff_dt.days > 0:
                valor.append(None)
                fecha.append(fecha_anterior + datetime.timedelta(minutes=int(frec_segundos/60)))
            valor.append(fila.valor)
            fecha.append(fila.fecha)
            fecha_anterior = fila.fecha
    ...
    for v in var_hidro:
        ...
        tabla = _model[fre.nombre].app + '_var' + str(v) + _model[fre.nombre].model.lower()
        sql = 'SELECT * FROM ' + tabla + ' WHERE estacion_id = %s AND fecha>= %s AND fecha<= %s '
        ...
        frecuencias = freq_hidro.filter(var_id=v).order_by('fre_fecha_ini').values_list('fre_fecha_ini', 'fre_valor', named=True)
        frecuencias = construir_intervalos_freq(frecuencias, inicio_datetime, fin_datetime)
        intervalo_frecuencia = frecuencias.pop()
        frec_segundos = intervalo_frecuencia['freq'] * 60
...
def construir_intervalos_freq(freqs, inicio, fin):
    ...
    for i in reversed(range(len(freqs))):
        if i == len(freqs) - 1:
            rangos.append({"inicio":freqs[i].fre_fecha_ini,
