...
class ConsultasPeriodoForm(forms.Form):
    ...
    inicio = forms.DateField(
        ...
        widget=forms.TextInput(
            attrs={"autocomplete": "off", "placeholder": "dd/mm/yy"}
        ),
    )
    fin = forms.DateField(
        input_formats=["%d/%m/%Y"],
    ...
    def __init__(self, *args, **kwargs):
        ...
        if is_authenticated:
            ...
            print("invitado")
            self.fields["estacion"] = forms.ModelChoiceField(
                queryset=Estacion.objects.filter(est_externa=False).order_by(
                    "est_codigo"
                )
            )
...
class ConsultasForm(forms.Form):
    ...
    estacion = forms.ModelChoiceField(
        queryset=Estacion.objects.all(), label="Estacion", required=False
    )
    ...
    variable = forms.ModelChoiceField(
        queryset=Variable.objects.filter(pk__in=variables), label="Variable", initial=1
    )
    inicio = forms.DateField(
        input_formats=["%d/%m/%Y"],
        label="Fecha de inicio",
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["frecuencia"] = forms.ChoiceField(
            choices=frecuencias, label="Frecuencia", initial=frecuencias[0][0]
        )
...
class ComparacionForm(forms.Form):
    ...
    variable = forms.ModelChoiceField(
        queryset=Variable.objects.order_by("var_id"), label="Variable", initial=1
    )
    inicio = forms.DateField(
        input_formats=["%Y-%m-%d"],
        label="Fecha de Inicio",
...
class ComparacionFormPublico(forms.Form):
    estacion = forms.MultipleChoiceField(
        choices=[], label="Estación", widget=forms.CheckboxSelectMultiple
    )
    lista_frecuencias = (
        # ('1', '5 Minutos'),
        ("2", "Horario"),
    ...
    def __init__(self, *args, **kwargs):
        ...
        estaciones = (
            Cruce.objects.filter(var_id=1, est_id__est_externa=False)
            .order_by("est_id")
            .distinct()
        )
        lista = []
...
class VariableForm(forms.Form):
    ...
    parametros_widget = forms.TextInput(
        attrs={"autocomplete": "off", "placeholder": "dd/mm/yy"}
    )
    format_input = ["%d/%m/%Y"]
    ...
    variable01 = forms.ModelChoiceField(
        queryset=var_con01, label="Variable Hidrológica"
    )
    ...
    inicio = forms.DateField(
        input_formats=format_input, label=lbl_inicio, widget=parametros_widget
    )
    fin = forms.DateField(
        input_formats=format_input, label=lbl_fin, widget=parametros_widget
    )
    frecuencia = forms.ChoiceField(choices=lista_frecuencias)
