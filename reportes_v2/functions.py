...
def comparar(form):
    ...
    for est_id in lista_estaciones:
        ...
        # frecuencia mensual
            ...
            informacion = datos_mensuales(estacion, variable, fecha_inicio, fecha_fin)
        if variable.var_id == 1:
            trace = get_elemento_data_json('bar', informacion['tiempo'], informacion['valor'], estacion.est_codigo)
        else:
            trace = get_elemento_data_json('scatter', informacion['tiempo'], informacion['valor'], estacion.est_codigo)
        data.append(trace)
...
def mapa_estaciones_variable(var_id, privado):
    ...
    if privado is True:
        ...
        estaciones = Cruce.objects.filter(var_id=var_id).order_by('est_id__est_codigo').distinct()
    else:
        '''estaciones = Estacion.objects.filter(
            est_id__in=Cruce.objects.filter(var_id=var_id).order_by().values('est_id_id').distinct(),
            est_externa=False)'''
        estaciones = Cruce.objects.filter(var_id=var_id, est_id__est_externa=False).order_by('est_id').distinct()
    ...
    for item in estaciones:
        fila = dict(
            ...
            geometry=dict(
                type='Point',
                coordinates=[float(item.est_id.est_longitud), float(item.est_id.est_latitud)]
            ),
            properties=dict(
                id=item.est_id.est_id,
