...
function operacion_valor(){
    es_acumulada = $("input[name='orig_variable_es_acumulada']").val();
    es_acumulada = (es_acumulada == 'true' || es_acumulada == 'True' || es_acumulada == 'TRUE');
    decimales = undefined;
    $(".seleccionado > .validacion").each(function(){
        valor_str = $(this).text().trim();
        valor_split = valor_str.split(".");
        if (valor_split.length == 2){
            decimales = valor_split[1].length;
            return false;
        }
    });

    if (es_acumulada){
        suma_valor(decimales);
    }else{
        promedio_valor(decimales);
    }
}


function suma_valor(decimales){
    $(".operacion_tipo").html("Suma: ");
    suma = null;
    $(".seleccionado").each(function(){
        valor = NaN;
        if ( ! $(this).children('.fecha').hasClass('error')){
            valor_str = $(this).children('.validacion').text().trim();
    ...
    if (typeof decimales !== 'undefined'){
        suma = suma.toFixed(decimales);
    }
    $(".operacion_valor").html(suma);
};


function promedio_valor(decimales){
    $(".operacion_tipo").html("Prom: ");
    suma = null;
    n = 0;
    $(".seleccionado").each(function(){
        valor = NaN;
    ...
    $(".operacion_valor").html(resultado);
};
...
function alarma_variacion(){
    var lim_sos = parseFloat($("input[name='orig_variacion_sos']").val());
    var lim_err = parseFloat($("input[name='orig_variacion_err']").val());
    var errores = 0;
    var sospechosos = 0;
    var normales = 0;

    $(".seleccionado > .var_con").each(function(){
        var variacion_txt = $(this).html().trim();
        var variacion = Math.abs(parseFloat(variacion_txt));
        if ( variacion < lim_sos ){
            $(this).addClass("normal");
            normales = normales + 1;
        }else if ( variacion < lim_err ){
            $(this).addClass("sospechoso");
            sospechosos = sospechosos + 1;
        }else if ( variacion >= lim_err ){
            $(this).addClass("error");
            errores = errores + 1 ;
        }else if (isNaN(variacion)){
            $(this).addClass("normal");
            normales = normales + 1;
        }
...
menu0_err_fechahora = "<div class='dropdown' id='habilitar_fecha'>"
    + "<a data-toggle='dropdown' data-boundary='window' aria-expanded='true'></a>"
    + "<div class='dropdown-menu' aria-labelledby='dropdownMenuButton'>"
    + "<a class='dropdown-item' onclick='habilitar_esta_fecha(this)' href='#/' >Habilitar esta fecha</a>"
    + "</div></div>";

menu0_fechahora = "<div class='dropdown' id='anular_fecha0'>"
    + "<a data-toggle='dropdown' data-boundary='window' aria-expanded='true'></a>"
    + "<div class='dropdown-menu' aria-labelledby='dropdownMenuButton'>"
    + "<a class='dropdown-item' onclick='anular_esta_fecha(this)' href='#/' >Anular esta fecha</a>"
    + "<a class='dropdown-item' onclick='anular_desde_aqui(this)' href='#/'>Anular desde aquí</a>"
    + "</div></div>";

menu0 = false;

menu1_fechahora = "<div class='dropdown' id='anular_fecha1'>"
    + "<a data-toggle='dropdown' data-boundary='window' aria-expanded='true'></a>"
    + "<div class='dropdown-menu' aria-labelledby='dropdownMenuButton'>"
    + "<a class='dropdown-item' onclick='anular_hasta_aca(this)' href='#/' >Anular hasta aquí</a>"
    + "<a class='dropdown-item' onclick='anular_cancelar(this)' href='#/'>Cancelar</a>"
    + "</div></div>";
...
function validacion_aceptar_valor(element){
    ...
    $("#id_comentario_dialog").dialog({
        ...
        open: function(event, ui) {
            event.target.parentElement.getElementsByClassName('ui-dialog-titlebar-close')[0].hidden = true;
        }
    });
}
...
function validacion_enviar(){
    ...
    profundidad = $("#id_profundidad").val();
    if (typeof profundidad !== 'undefined'){
        //Existe valor profundidad
        $(".seleccionado > .fecha").each(function(){
            fila = $(this).parent();
            ...
            cambios.push({fecha: fecha, profundidad: profundidad, valor: valor, comentario: comentario});
        });
    }else{
        //Variable SIN profundidad
        $(".seleccionado > .fecha").each(function(){
            fila = $(this).parent();
            fecha = $(this).text().trim();
            if ($(this).hasClass('error')){
...
$(document).ready(function() {
    ...
    $('.seleccionado > .validacion').dblclick(function(){
        ...
        boton_ok = '<button type="button" class="btn btn-success btn-xs" onclick="validacion_aceptar_valor(this)"><i class="fas fa-check fa-sm fa-fw" ></i></button>';
        boton_no = '<button type="button" class="btn btn-danger btn-xs" onclick="validacion_cancelar_valor(this)"><i class="fas fa-times fa-sm fa-fw" ></i></button>';
