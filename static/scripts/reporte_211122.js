...
function graficar_datos(d){
    var data = [];
    var trace = {};
    var pixels_por_dato = 1.2;
    var layout_height = 500;
    var layout_yaxis_title = d.variable.nombre + ' [' + d.variable.unidad_sigla +
        ((d.frecuencia === "") ? '' : '/' + d.frecuencia) +']';
    var layout_grid = {
        rows: 1,
        columns: 1,
    };
    var layout_yaxis_domain = [0, 1];
    var layout_yaxis2_domain = [1, 1];
    ...
    if (d.variable.es_acumulada){
        trace = {
            name: d.variable.unidad_sigla,
            type: 'bar',
            x: d.fecha,
            y: d.valor,
            width: 60000 * d.periodo,
            marker: {
    ...
    if (!d.excluir_vacios){
        vacios = {
            ...
            width: 60000 * d.periodo,
            marker: {
                line: {'width':0.5, 'color':'rgb(200,0,0)'}
            },
            showlegend: false,
        }
        data.push(vacios);
        layout_yaxis_domain = [0.3, 1];
        layout_yaxis2_domain = [0, 0.2];
    }


    var layout = {
        title: d.estacion.codigo,
        width: 160 + (d.ndatos_esperado * pixels_por_dato),
