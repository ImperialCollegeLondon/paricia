    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            if self.request.is_ajax():
                ...
                fechaa= modelo.objects.filter(estacion_id__exact = form.data['estacion'] ).aggregate(Max('fecha'),Min('fecha'))
...
class ListaValidacion(LoginRequiredMixin, ListView):
    ...
    def get(self, request, *args, **kwargs):
        if self.request.is_ajax():
            ...
            datos = functions.consultar_diario(est_id, var_id, fecha_str, maximo, minimo)
...
def guardar_crudos(request):
    ...
    #    cursor.close()
        ...
        with connection.cursor() as cursor:
            ...
            cursor.callproc('insertar_' + variable_nombre.lower() + '_validacion', [estacion_id, cambios_json])
            resultado = cursor.fetchone()[0]
            print(resultado)
        cursor.close()
