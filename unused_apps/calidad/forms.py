...
class Grafico1Form(forms.Form):
    estacion = forms.ModelChoiceField(
        queryset=Estacion.objects.none(), label="Estacion", required=False
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        estaciones = Estacion.objects.filter(
            pk__in=Cruce.objects.filter(var_id__var_id__in=vars_calidad)
            .order_by()
            .values_list("est_id")
            .distinct()
        ).distinct()
...
class AsociacionHidroForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            "estaciones_hidro": admin.widgets.FilteredSelectMultiple(
                "Estaciones Hidrología", is_stacked=False
            ),
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        estacion_calidad = Estacion.objects.filter(
            tipo__nombre="Calidad de agua"
        ).distinct()
        self.fields["estacion_calidad"].queryset = estacion_calidad
        estaciones_hidro = Estacion.objects.exclude(
            tipo__nombre__in=["Calidad de agua", "Aforo"]
        ).distinct()
...
class CompararHidroForm(forms.Form):
    ...
    frecuencia = forms.ModelChoiceField(
        queryset=TipoFrecuencia.objects.all().order_by("id"),
        label="Frecuencia",
        required=True,
    )
    inicio = forms.DateField(
    ...
    est_calidad = forms.ModelChoiceField(
        queryset=Estacion.objects.none(), label="Est. Calidad"
    )
    PROFUNDIDADES = [
        (None, "---------"),
        ("30", "0.3 m"),
    ...
    profundidad = forms.ChoiceField(choices=PROFUNDIDADES)
    var_calidad = forms.MultipleChoiceField(
        choices=[], widget=forms.CheckboxSelectMultiple
    )

    est_hidro = forms.ModelChoiceField(
        queryset=Estacion.objects.none(), label="Est. Hidro"
    )
    var_hidro = forms.MultipleChoiceField(
        choices=[], widget=forms.CheckboxSelectMultiple
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        asociacion_hidro = models.AsociacionHidro.objects.get(
            estacion_calidad=est_calidad1
        )
...
class UsuarioVariableForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            "variable": admin.widgets.FilteredSelectMultiple(
                "Variable", is_stacked=False
            ),
        }
