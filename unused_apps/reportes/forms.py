...
from estacion.models import Cuenca, Estacion, Sitio
from frecuencia.models import TipoFrecuencia, UsuarioTipoFrecuencia
...
class AnuarioForm(forms.Form):
    ...
    anio = forms.ChoiceField(
        required=False, choices=años, label="Año", initial=año_actual
    )
...
class DiarioForm(forms.Form):
    filtro = forms.ChoiceField(
        choices=FILTRO, widget=forms.RadioSelect(), initial=FILTRO[0][0], label="Filtro"
    )
    estacion_id = forms.IntegerField(required=False, widget=forms.HiddenInput())
    sitio = forms.ModelChoiceField(
        queryset=Sitio.objects.order_by("id").all(), label="Sitio", required=False
    )
    cuenca = forms.ModelChoiceField(
        queryset=Cuenca.objects.order_by("id").all(), label="Cuenca", required=False
    )
    variable = forms.ModelChoiceField(
        queryset=Variable.objects.filter(pk__in=var_diario),
        label="Variable",
        initial=var_diario[0],
        required=True,
    )
    año = forms.ChoiceField(
        choices=años, label="Año", initial=año_actual, required=True
    )
...
class MensualMultianualForm(forms.Form):
    ...
    variable = forms.ModelChoiceField(
        queryset=Variable.objects.filter(pk__in=var_mensualmultianual),
        label="Variable",
        initial=var_mensualmultianual[0],
        required=True,
    )
    año_inicio = forms.ChoiceField(
        choices=años, label="Año inicio", initial=años[0], required=False
    )
    año_fin = forms.ChoiceField(
        choices=años, label="Año fin", initial=año_actual, required=False
    )


class ConsultasForm(forms.Form):
    frecuencia = forms.ModelChoiceField(
        queryset=TipoFrecuencia.objects.none(), label="Frecuencia", required=True
    )
    filtro = forms.ChoiceField(
        choices=FILTRO,
        widget=forms.RadioSelect(),
        initial=FILTRO[0][0],
        label="Filtro Tipo Estación",
        required=False,
    )
    estacion_id = forms.IntegerField(required=False, widget=forms.HiddenInput())
    sitio = forms.ModelChoiceField(
    ...
    cuenca = forms.ModelChoiceField(
        queryset=Cuenca.objects.none(), label="Cuenca", required=False
    )
    variable = forms.ModelChoiceField(
        queryset=Variable.objects.filter(pk__in=var_hidro_consulta_periodo),
        label="Variable",
        initial=1,
        required=True,
    )
    ...
    excluir_vacios = forms.BooleanField(
        label="Excluir vacíos que no cumplan el umbral.", initial=True, required=False
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if user.username == "admin":
            ...
            try:
                ...
                frecuencia_queryset = TipoFrecuencia.objects.filter(
                    pk__in=uf.values("tipofrecuencia_id")
                ).order_by("id")
            except:
...
class ComparacionForm(forms.Form):
    estacion01 = forms.ModelChoiceField(
        required=False,
        queryset=Estacion.objects.order_by("est_id").all(),
        label="Primera Estación",
    )
    estacion02 = forms.ModelChoiceField(
        required=False,
        queryset=Estacion.objects.order_by("est_id").all(),
        label="Segunda Estación",
    )
    estacion03 = forms.ModelChoiceField(
        required=False,
        queryset=Estacion.objects.order_by("est_id").all(),
...
class VariableForm(forms.Form):
    ...
    variable01 = forms.ModelChoiceField(
        required=False,
        queryset=Variable.objects.order_by("var_id").all(),
    ...
    variable02 = forms.ModelChoiceField(
        required=False,
        queryset=Variable.objects.order_by("var_id").all(),
