...
def reporte_excel(form):
    ...
    if len(maximos_abs) != maximos_abs.count(None) and len(
        maximos_pro
    ) != maximos_pro.count(None):
        ws["C9"] = "Max absoluto"
        ws["D9"] = "Min absoluto"
        ws["E9"] = "Max promedio"
        ws["F9"] = "Min promedio"
        for valor, maximo_abs, maximos_pro, minimo_abs, minimo_pro, fecha in zip(
            valores, maximos_abs, maximos_pro, minimos_abs, minimos_pro, tiempo
        ):
        ...
        ws["C9"] = "Max promedio"
        ws["D9"] = "Min promedio"
        for valor, maximos_pro, minimo_pro, fecha in zip(
            valores, maximos_pro, minimos_pro, tiempo
        ):
            ws.cell(row=cont, column=1).value = fecha
            ws.cell(row=cont, column=2).value = valor
...
def datos_instantaneos(estacion, variable, fecha_inicio, fecha_fin):
    ...
    fecha_fin = datetime(
        fecha_fin.year, fecha_fin.month, fecha_fin.day, 23, 59, 59, 999999
    )
...
def datos_estacion(estacion, fecha_inicio, fecha_fin):
    ...
    for fila in cruce:
        valor, maximo, minimo, frecuencia = datos_instantaneos(
            estacion, fila.var_id, fecha_inicio, fecha_fin
        )
...
def armar_consulta(estacion, variable, frecuencia, fecha_inicio, fecha_fin):
    ...
    if (variable.var_id == 4 or variable.var_id == 5) and estacion.est_externa is False:
        ...
        variable.var_id == 10 or variable.var_id == 11
    ) and estacion.est_externa is False:
    ...
    if variable.var_id == 1:
        ...
        sql = "SELECT " + fecha
        sql += (
            "AVG(direccion) as valor"
            + coma
            + "MAX(direccion) as max_pro"
            + coma
            + "MIN(direccion) as min_pro"
        )
        sql += " FROM " + tabla + " WHERE "
        sql += filtro + group_order
        ...
        sql = "SELECT " + fecha
        sql += (
            promedio + coma + max_abs + coma + max_pro + coma + min_abs + coma + min_pro
        )
        sql += " FROM " + tabla + " WHERE "
        sql += filtro + group_order
...
def datos_horarios_json(est_id, var_id, fec_ini, fec_fin):
    ...
    if len(val) > 0:
        for (
            ...
            if (
                fecha_ini
                <= datetime.strptime(item_time, "%Y-%m-%d %H:%M:%S")
                <= fecha_fin
            ):
                dato = {
                    "fecha": item_time,
