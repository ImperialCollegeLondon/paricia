...
class TypeVI(Titulos):
    ...
    def matriz(estacion, variable, periodo):
        datos = {}
        rad_min = list(
            RadiacionMinima.objects.filter(est_id=estacion).filter(rad_periodo=periodo)
        )
        rad_max = list(
            RadiacionMaxima.objects.filter(est_id=estacion).filter(rad_periodo=periodo)
        )
        if rad_min and rad_max:
            datos = {"rad_max": rad_max, "rad_min": rad_min}
    ...
    def tabla_excel(self, ws, estacion, variable, periodo, tipo):
        ...
        if tipo == "maxima":
            ...
            subtitle.value = (
                "Radiación Solar - Valores máximos, mínimos medios horarios"
            )
            self.set_style(
                cell=subtitle,
                font="font_bold_10",
