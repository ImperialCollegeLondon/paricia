...
from .functions import dia_hoy, primer_dia_mes_actual
...
class ConsultaForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        estaciones = Estacion.objects.filter(
            pk__in=ConfigVisualizar.objects.values("estacion_id")
        ).distinct()
...
class AlarmaTransmisionLimitesForm(forms.Form):
    lim_inf = forms.DecimalField(
        max_digits=4, decimal_places=1, required=True, label="Límite 1, EXPECTANTE"
    )
    lim_sup = forms.DecimalField(
        max_digits=4, decimal_places=1, required=True, label="Límite 2, FALLO"
    )

    def __init__(self, *args, **kwargs):
        super(AlarmaTransmisionLimitesForm, self).__init__(*args, **kwargs)
        self.fields["lim_inf"].initial = round(
            TeleVariables.objects.get(nombre="ALAR_TRAN_LIMI_INFE").valor, 1
        )
        self.fields["lim_sup"].initial = round(
            TeleVariables.objects.get(nombre="ALAR_TRAN_LIMI_SUPE").valor, 1
        )
