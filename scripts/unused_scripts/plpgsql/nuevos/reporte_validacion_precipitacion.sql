-- FUNCTION: public.reporte_validacion_precipitacion(integer, timestamp with time zone, timestamp with time zone, numeric, numeric)

-- DROP FUNCTION public.reporte_validacion_precipitacion(integer, timestamp with time zone, timestamp with time zone, numeric, numeric);
...
CREATE OR REPLACE FUNCTION public.reporte_validacion_precipitacion(
	...
	_var_maximo numeric,
	_var_minimo numeric)
    RETURNS TABLE(id bigint, fecha timestamp with time zone, valor numeric, validado boolean, seleccionado boolean, estado boolean, fecha_error numeric, valor_error boolean, stddev_error boolean, comentario character varying, variacion_consecutiva numeric, varia_error boolean) 
    LANGUAGE 'plpgsql'
...
BEGIN
	...
	reporte AS (
		SELECT ts.numero_fila AS id, ts.fecha, ts.valor, ts.existe_en_validacion, ts.seleccionado,
			...
			ts.variacion_consecutiva <= -(select var_err from variable where var_id = 1) as varia_error
...
--ALTER FUNCTION public.reporte_validacion_precipitacion(integer, timestamp with time zone, timestamp with time zone, numeric, numeric)
--    OWNER TO usuario1;
