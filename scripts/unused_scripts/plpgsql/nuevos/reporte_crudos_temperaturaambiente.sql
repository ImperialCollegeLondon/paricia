-- FUNCTION: public.reporte_crudos_temperaturaambiente(integer, timestamp with time zone, timestamp with time zone, numeric, numeric)

-- DROP FUNCTION public.reporte_crudos_temperaturaambiente(integer, timestamp with time zone, timestamp with time zone, numeric, numeric);

CREATE OR REPLACE FUNCTION public.reporte_crudos_temperaturaambiente(
...
BEGIN
    ...
    validacion AS (
        ...
        FROM validacion_var2validado v WHERE v.estacion_id = (SELECT est_id FROM estacion)
    ...
    medicion AS (
        ...
        FROM medicion_var2medicion m WHERE m.estacion_id = (SELECT est_id FROM estacion)
...
-- ALTER FUNCTION public.reporte_crudos_temperaturaambiente(integer, timestamp with time zone, timestamp with time zone, numeric, numeric)
--    OWNER TO usuario1;
