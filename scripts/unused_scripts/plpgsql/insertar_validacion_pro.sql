...
DROP FUNCTION IF EXISTS  public.insertar_%%modelo%%_validacion(bigint, json);

CREATE OR REPLACE FUNCTION public.insertar_%%modelo%%_validacion(
	_estacion_id bigint,
	_datos json)
    RETURNS boolean
    LANGUAGE 'plpgsql'
...
--ALTER FUNCTION public.insertar_%%modelo%%_validacion(bigint, json)
--    OWNER TO usuario1;
