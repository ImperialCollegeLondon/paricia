DROP FUNCTION  IF EXISTS  public.reporte_crudos_agua(integer, timestamp with time zone, timestamp with time zone, numeric, numeric);
...
CREATE OR REPLACE FUNCTION public.reporte_crudos_agua(
	...
	_var_maximo numeric,
	_var_minimo numeric)
    RETURNS TABLE(id bigint, fecha timestamp with time zone, nivel numeric, caudal numeric,
