...
funcion_validacion = """
DROP FUNCTION IF EXISTS public.reporte_validacion_diario_%%variable%%(integer, timestamp with time zone, timestamp with time zone);
CREATE OR REPLACE FUNCTION public.reporte_validacion_diario_%%variable%%(
    ...
    RETURNS TABLE(numero_fila bigint, dia timestamp with time zone, 
        ...
        class_valor text, class_maximo text, class_minimo text, class_porcentaje text)
    LANGUAGE 'plpgsql'
...
ALTER FUNCTION public.reporte_validacion_diario_%%variable%%(integer, timestamp with time zone, timestamp with time zone)
    OWNER TO postgres;

"""

funcion_precipitacion = """
DROP FUNCTION IF EXISTS public.reporte_validacion_diario_precipitacion(integer, timestamp with time zone, timestamp with time zone);
CREATE OR REPLACE FUNCTION public.reporte_validacion_diario_precipitacion(
	_estacion_id integer,
	_fecha_inicio timestamp with time zone,
	_fecha_fin timestamp with time zone)
    RETURNS TABLE(numero_fila bigint, dia timestamp with time zone, 
    	valor numeric, porcentaje numeric, class_valor text, class_porcentaje text)
    LANGUAGE 'plpgsql'
...
BEGIN
    ...
    --SELECT * FROM fechas0 order by fecha_grupo
    SELECT rc.numero_fila,rc.dia, rc.valor, rc.porcentaje, rc.class_valor, rc.class_porcentaje FROM reporte_css rc;
...
for index, var in enumerate(variables):
    ...
    if index == 0:
        cursor.execute(funcion_precipitacion)
    else:
        sql_fun = funcion_validacion.replace('%%variable%%', var)
        sql_fun = sql_fun.replace('%%var_id%%', str(index+1))
        cursor.execute(sql_fun)