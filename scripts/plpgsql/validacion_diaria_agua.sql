DROP FUNCTION IF EXISTS  public.reporte_validacion_diario_agua(integer, timestamp with time zone, timestamp with time zone, numeric, numeric);
CREATE OR REPLACE FUNCTION public.reporte_validacion_diario_agua(
    ...
    RETURNS TABLE(id bigint, fecha timestamp with time zone, fecha_error numeric, fecha_numero numeric,
        ...
        nivel_error boolean,
        nivel_numero numeric,
        caudal_error boolean,
        caudal_numero numeric,
        estado boolean, validado boolean)
    LANGUAGE 'plpgsql'
