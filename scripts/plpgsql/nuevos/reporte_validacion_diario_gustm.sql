-- FUNCTION: public.reporte_validacion_diario_gustm(integer, timestamp with time zone, timestamp with time zone, numeric, numeric)

-- DROP FUNCTION public.reporte_validacion_diario_gustm(integer, timestamp with time zone, timestamp with time zone, numeric, numeric);

CREATE OR REPLACE FUNCTION public.reporte_validacion_diario_gustm(
...
BEGIN
	...
	variable AS (SELECT * FROM variable_variable var WHERE var.var_id = 19),
    ...
    validacion AS (
        ...
        FROM validacion_var19validado v WHERE v.estacion_id = (SELECT est_id FROM estacion) AND v.fecha >= _fecha_inicio AND v.fecha <= _fecha_fin
    ...
    medicion AS (
        ...
        FROM medicion_var19medicion m WHERE m.estacion_id = (SELECT est_id FROM estacion) AND m.fecha >= _fecha_inicio AND m.fecha <= _fecha_fin
    ),
    ...
    tabla_varia_err AS (
        SELECT ta.dia,
            (SELECT COUNT(tv.variacion_consecutiva) nsvalor FROM tabla_variacion tv WHERE date(tv.fecha) = ta.dia
                AND (tv.variacion_consecutiva <= -(select abs(var_err) from variable where var_id = 19) )
    ...
    reporte AS (
        ...
        ROUND((SELECT AVG(dp.valor) FROM diario_var19diario dp WHERE dp.estacion_id = (SELECT est_id FROM estacion) AND
...
-- ALTER FUNCTION public.reporte_validacion_diario_gustm(integer, timestamp with time zone, timestamp with time zone, numeric, numeric)
--    OWNER TO usuario1;
