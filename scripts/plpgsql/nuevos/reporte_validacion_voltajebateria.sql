-- FUNCTION: public.reporte_validacion_voltajebateria(integer, timestamp with time zone, timestamp with time zone, numeric, numeric)

-- DROP FUNCTION public.reporte_validacion_voltajebateria(integer, timestamp with time zone, timestamp with time zone, numeric, numeric);

CREATE OR REPLACE FUNCTION public.reporte_validacion_voltajebateria(
...
BEGIN
	...
	reporte AS (
		SELECT ts.numero_fila AS id, ts.fecha, ts.valor, ts.maximo, ts.minimo, ts.existe_en_validacion, ts.seleccionado,
			...
			ts.variacion_consecutiva <= -(select var_err from variable where var_id = 12) as varia_error
...
-- ALTER FUNCTION public.reporte_validacion_voltajebateria(integer, timestamp with time zone, timestamp with time zone, numeric, numeric)
--    OWNER TO usuario1;
