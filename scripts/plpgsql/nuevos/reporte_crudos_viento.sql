-- FUNCTION: public.reporte_crudos_viento(integer, timestamp with time zone, timestamp with time zone, numeric, numeric)

-- DROP FUNCTION public.reporte_crudos_viento(integer, timestamp with time zone, timestamp with time zone, numeric, numeric);
...
CREATE OR REPLACE FUNCTION public.reporte_crudos_viento(
	...
	_var_maximo numeric,
	_var_minimo numeric)
    RETURNS TABLE(id bigint, fecha timestamp with time zone, valor numeric, maximo numeric, minimo numeric, direccion numeric, categoria numeric, estado boolean, seleccionado boolean, comentario character varying) 
    LANGUAGE 'plpgsql'
...
-- ALTER FUNCTION public.reporte_crudos_viento(integer, timestamp with time zone, timestamp with time zone, numeric, numeric)
--    OWNER TO usuario1;
