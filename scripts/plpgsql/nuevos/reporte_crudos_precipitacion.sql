-- FUNCTION: public.reporte_crudos_precipitacion(integer, timestamp with time zone, timestamp with time zone, numeric, numeric)

-- DROP FUNCTION public.reporte_crudos_precipitacion(integer, timestamp with time zone, timestamp with time zone, numeric, numeric);

CREATE OR REPLACE FUNCTION public.reporte_crudos_precipitacion(
	_estacion_id integer,
	_fecha_inicio timestamp with time zone,
	_fecha_fin timestamp with time zone,
	_var_maximo numeric,
	_var_minimo numeric)
    RETURNS TABLE(id bigint, fecha timestamp with time zone, valor numeric, estado boolean, seleccionado boolean, comentario character varying) 
    LANGUAGE 'plpgsql'
...
BEGIN
	RETURN QUERY
	--pruebas_reporte_crudos_acumulado.sql
    WITH
	...
	tabla_base AS (
		SELECT m.fecha, m.valor FROM medicion m WHERE m.existe_en_validacion = FALSE
		GROUP BY m.fecha, m.valor
		HAVING COUNT(*)=1
	),
	reporte AS (
...
-- ALTER FUNCTION public.reporte_crudos_precipitacion(integer, timestamp with time zone, timestamp with time zone, numeric, numeric)
--    OWNER TO usuario1;
