...
BEGIN
	...
	tabla1 AS (
		SELECT *,
			...
			CASE
			    WHEN tb.numero_fila = 1 THEN TRUE ELSE CASE
	...
	tabla2 AS (
		SELECT *,
			(SELECT t1.valor - (SELECT tanterior.valor FROM tabla1 tanterior
			                    WHERE tanterior.fecha_grupo = t1.fecha_grupo - 1 AND tanterior.seleccionado IS TRUE)
            ) AS variacion_consecutiva,
	...
	reporte AS (
		SELECT *,
			...
			t2.valor < (SELECT lim_inf_stddev FROM estadistica ) OR t2.valor > (SELECT lim_sup_stddev FROM estadistica)
			    AS stddev_error,
			CASE
	...
	SELECT rf.numero_fila, rf.seleccionado, rf.fecha, rf.valor_seleccionado, rf.valor, rf.variacion_consecutiva,
	    rf.comentario, rf.class_fila, rf.class_fecha, rf.class_validacion, rf.class_valor,
	    rf.class_variacion_consecutiva, rf.class_stddev_error FROM reporte_con_clases_para_html rf;
END; $$
LANGUAGE 'plpgsql';
