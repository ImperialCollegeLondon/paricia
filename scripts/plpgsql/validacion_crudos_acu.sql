...
CREATE OR REPLACE FUNCTION public.reporte_validacion_precipitacion(
    ...
    RETURNS TABLE(id bigint, fecha timestamp with time zone, valor numeric, existe_en_validacion boolean,
...
BEGIN
	...
	reporte AS (
		SELECT ts.numero_fila AS id, ts.fecha, ts.valor, ts.existe_en_validacion, ts.estado,
