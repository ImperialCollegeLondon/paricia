DROP FUNCTION IF EXISTS generar_diario_precipitacion();
CREATE OR REPLACE FUNCTION generar_diario_precipitacion()
  RETURNS boolean AS
$BODY$
...
BEGIN
    ...
    dato AS (
        SELECT * FROM horario_precipitacion h
    ...
    update_diario AS (
        UPDATE diario_precipitacion SET
    ...
    insert_diario AS (
        INSERT INTO diario_precipitacion(estacion_id, fecha, valor, completo_mediciones, completo_umbral, usado_para_mensual)
    ...
    update_USADO_PARA_DIARIO AS (
        UPDATE horario_precipitacion SET usado_para_diario = TRUE
