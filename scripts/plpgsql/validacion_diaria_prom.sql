DROP FUNCTION IF EXISTS  public.reporte_validacion_diario_modelo(integer, timestamp with time zone, timestamp with time zone, numeric, numeric);
...
CREATE OR REPLACE FUNCTION public.reporte_validacion_diario_modelo(
    ...
    RETURNS TABLE(id bigint, fecha timestamp with time zone, fecha_error numeric, fecha_numero numeric,
    	valor numeric, maximo numeric, minimo numeric, porcentaje numeric, porcentaje_error boolean,
        valor_error boolean, maximo_error boolean, minimo_error boolean,
        valor_numero numeric, maximo_numero numeric, minimo_numero numeric, media_historica numeric, estado boolean, validado boolean, c_varia_err numeric)
    LANGUAGE 'plpgsql'
...
BEGIN
    ...
    validacion AS (
        ...
        FROM validacion_%%variable%%validado v WHERE v.estacion_id = (SELECT est_id FROM estacion) AND v.fecha >= _fecha_inicio AND v.fecha <= _fecha_fin
    ...
    medicion AS (
        ...
        FROM medicion_%%variable%%medicion m WHERE m.estacion_id = (SELECT est_id FROM estacion) AND m.fecha >= _fecha_inicio AND m.fecha <= _fecha_fin
    ),
    ...
    tabla_varia_err AS (
        SELECT ta.dia,
            (SELECT COUNT(tv.variacion_consecutiva) nsvalor FROM tabla_variacion tv WHERE date(tv.fecha) = ta.dia
                AND (tv.variacion_consecutiva <= -(select abs(var_err) from variable where var_id = %%var_id%%) )
    ...
    reporte AS (
        ...
        ROUND((SELECT AVG(dp.valor) FROM diario_%%variable%%diario dp WHERE dp.estacion_id = (SELECT est_id FROM estacion) AND
...
ALTER FUNCTION public.reporte_validacion_diario_modelo(integer, timestamp with time zone, timestamp with time zone, numeric, numeric)
    OWNER TO usuario1;