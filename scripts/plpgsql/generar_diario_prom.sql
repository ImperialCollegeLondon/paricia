...
-- ---##############################################################
-- ---####  funcion para generar datos diarios de nivelagua
-- ---##############################################################

DROP FUNCTION IF EXISTS generar_diario_nivelagua();
CREATE OR REPLACE FUNCTION generar_diario_nivelagua()
  RETURNS boolean AS
$BODY$
DECLARE _resultado boolean;
BEGIN
    ...
    insert_diario AS (
        INSERT INTO diario_nivelagua(estacion_id, fecha, valor, max_abs, min_abs, max_del_prom, min_del_prom, completo_mediciones, completo_umbral, usado_para_mensual)
        SELECT estacion_id, fecha, valor, max_abs, min_abs, max_del_prom, min_del_prom, completo_mediciones, completo_umbral, usado_para_mensual
        FROM calculo
        WHERE NOT EXISTS (SELECT 1 FROM update_diario)
        AND (SELECT es_valido FROM es_valido)
    ...
    update_USADO_PARA_DIARIO AS (
        UPDATE horario_nivelagua SET usado_para_diario = TRUE
        WHERE id = ANY(ARRAY(SELECT b.id FROM bloque b))
        AND (EXISTS (SELECT 1 FROM update_diario) OR EXISTS (SELECT 1 FROM insert_diario))
        RETURNING *
    ),
