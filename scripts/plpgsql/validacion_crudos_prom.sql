DROP FUNCTION IF EXISTS  public.reporte_validacion_%%modelo%%(integer, timestamp with time zone, timestamp with time zone, numeric, numeric);
...
BEGIN
	...
	reporte AS (
		SELECT ts.numero_fila AS id, ts.fecha, ts.valor, ts.maximo, ts.minimo, ts.existe_en_validacion, ts.seleccionado,
			...
			ts.variacion_consecutiva <= -(select var_err from variable where var_id = %%var_id%%) as varia_error
