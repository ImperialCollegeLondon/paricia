DROP FUNCTION IF EXISTS generar_horario_var1();
CREATE OR REPLACE FUNCTION generar_horario_var1()
  RETURNS boolean AS
$BODY$
...
BEGIN
    WITH
    dato AS (
        SELECT * FROM validacion_var1validado v
    ...
    calculo AS (
        -- Calcula la suma/acumulado del valor de todo el bloque de validados
        SELECT
            (SELECT d.estacion_id FROM dato d) AS estacion_id,
            (SELECT h.hora_inicio FROM hora_inicio h) AS fecha,
            (SELECT SUM(v.valor) FROM validados v WHERE v.valor IS NOT NULL) AS valor,
    ...
    es_valido AS (
        SELECT
            (fecha IS NOT NULL) AND (vacios <= 100.0) AND (vacios >= 0.0) AS es_valido
        FROM calculo
    ),
    update_horario AS (
        -- Primera opción es actualizar el dato horario en caso de que ya hubiese existido
        UPDATE horario_var1horario SET
    ...
    insert_horario AS (
        ...
        INSERT INTO horario_var1horario(estacion_id, fecha, valor, vacios, usado_para_diario)
        SELECT estacion_id, fecha, valor, vacios, usado_para_diario FROM calculo
    ...
    update_USADO_PARA_HORARIO_validados AS (
        ...
        UPDATE validacion_var1validado SET usado_para_horario = TRUE
