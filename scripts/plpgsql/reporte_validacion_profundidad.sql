DROP FUNCTION IF EXISTS reporte_validacion_profundidad_var101(integer, integer, timestamp with time zone,timestamp with time zone);
CREATE OR REPLACE FUNCTION reporte_validacion_profundidad_var101(_estacion_id INT, _profundidad INT, _fecha_inicio TIMESTAMP WITH TIME ZONE, _fecha_fin TIMESTAMP WITH TIME ZONE)
RETURNS TABLE (
	numero_fila BIGINT,
	seleccionado BOOL,
...
BEGIN
	...
	variable AS (SELECT * FROM variable_variable var WHERE var.var_id = 101),
	validacion AS (
		SELECT v.id, v.fecha, 0 AS tipo, v.valor, TRUE AS existe_en_validacion
		FROM validacion_var101validado v
		WHERE v.estacion_id = (SELECT est_id FROM estacion) AND v.profundidad = _profundidad
		AND v.fecha >= _fecha_inicio AND v.fecha <= _fecha_fin
		ORDER BY v.fecha
	),
	medicion AS (
		SELECT m.id, m.fecha, 1 AS tipo, m.valor, EXISTS(SELECT * FROM validacion v WHERE v.fecha = m.fecha AND v.valor = m.valor) AS existe_en_validacion
		FROM medicion_var101medicion m
		WHERE m.estacion_id = (SELECT est_id FROM estacion) AND m.profundidad = _profundidad
	...
	fechas0 AS (
		SELECT
			ff.fecha,
			row_number() OVER (ORDER BY ff.fecha ASC) as grupo_fecha,
		...
		FROM (SELECT DISTINCT(date_trunc('minute', umv.fecha) ) AS fecha FROM union_med_val umv) ff ORDER BY fecha ASC
	),
	fechas AS (
		SELECT *,
			CASE WHEN grupo_fecha = 1 THEN 1 ELSE
			CASE WHEN lapso_tiempo < periodo_esperado THEN 0
				 WHEN lapso_tiempo > periodo_esperado THEN 2
				ELSE 1
			END
		END AS fecha_valida
		FROM fechas0
	...
	tabla1 AS (
		SELECT *,
			(SELECT grupo_fecha FROM fechas f WHERE f.fecha = date_trunc('minute', tb.fecha)) AS grupo_fecha,
			CASE WHEN
			    tb.numero_fila = 1 THEN TRUE
			ELSE CASE WHEN
			    date_trunc('minute', lag(tb.fecha) OVER (ORDER BY tb.numero_fila ASC)) != date_trunc('minute', tb.fecha) THEN TRUE ELSE FALSE
			END END AS seleccionado,
			(SELECT med.id FROM medicion med WHERE med.fecha = tb.fecha ORDER BY id ASC LIMIT 1) AS medicion_id
		FROM tabla_base tb
	),
	tabla2 AS (
		SELECT *,
			(SELECT t1.valor - (SELECT tanterior.valor FROM tabla1 tanterior WHERE tanterior.grupo_fecha = t1.grupo_fecha - 1 AND tanterior.seleccionado IS TRUE) ) AS variacion_consecutiva,
	...
	reporte AS (
		SELECT *,
			(SELECT fecha_valida FROM fechas ff WHERE ff.fecha = date_trunc('minute', t2.fecha)) AS fecha_error,
	...
	reporte_con_clases_para_html AS (
		SELECT *,
			CASE
			    WHEN r.seleccionado THEN CAST(grupo_fecha AS text) || ' seleccionado'
	...
	SELECT rf.numero_fila, rf.seleccionado, rf.fecha, rf.valor_seleccionado, rf.valor, rf.variacion_consecutiva, rf.comentario, rf.class_fila, rf.class_fecha, rf.class_validacion,
		rf.class_valor, rf.class_variacion_consecutiva, rf.class_stddev_error
	FROM reporte_con_clases_para_html rf order by rf.numero_fila;
END; $$
LANGUAGE 'plpgsql';