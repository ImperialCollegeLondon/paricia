        ...
        try:
            ...
            for formato in formatos:
                ...
                if len(consulta) > 0:
                    ...
                    registro = open('/tmp/sedc.txt', 'a')
                    registro.write(
                        time.ctime() + ': Lectura Iniciada - Estacion:' + str(estacion.est_codigo) + ' Formato: ' + str(
                    ...
                    if len(datos) > 0:
                        ...
                        registro = open('/tmp/sedc.txt', 'a')
                        registro.write(time.ctime() + ': Información guardada Estacion:' + str(
                            estacion.est_codigo) + 'Formato:' + str(formato.for_descripcion) + '\n')
                        registro.close()

                    else:
                        registro = open('/tmp/sedc.txt', 'a')
                        registro.write(time.ctime() + ': No existe nueva informacion para el Formato: ' + str(
                            formato.for_descripcion) + '\n')
                        registro.close()
                else:
                    registro = open('/tmp/sedc.txt', 'a')
