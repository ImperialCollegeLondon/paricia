...
class SitioCuencaList(PermissionRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        campos = ['id', 'cuenca_id__nombre', 'sitio_id__nombre']
...
def datos_json_estaciones(request):
    ...
    for item in estaciones:
        if (type(item.est_latitud) is not type(None)) and (type(item.est_longitud) is not type(None)) :
...
def listar_anio(request, estacion, var):
    ...
    validados = modelo.objects.filter(estacion_id__exact=estacion).aggregate(Max('fecha'), Min('fecha'))
