...
def indicaCaudal(estacion_id, inicio, fin, completo):
    ...
    if amax is not None and amin and len(datos) > 2:
        ...
        res_caudal = mes.Var10Mensual.objects.filter(estacion_id__exact=est.est_id, fecha__gte=inicio,
                                              fecha__lte=fin).order_by('fecha').values("id", "fecha", "valor")
        ...
        if inf is not None:
            ...
            dic = {"cmax": str(round(camax, 3)), "fdmax": fdmax[0]['fecha'].strftime("%d/%m/%Y"),
                  ...
                  "cmessec_es": str(round(caSeco/inf, 3)), "fecmessec_es": fecmessec[0].fecha.strftime("%m/%Y"),
                   "dias_sin_caudal":dias_sin_caudal, "tabla_caudal":tabla_caudal,
                }
...
def calcular_escorrentia(sitiocuenca, est_caudal, est_precipitacion, inicio, fin):
    ...
    res = mes.Var10Mensual.objects.filter(estacion_id__exact=est_caudal.est_id, fecha__gte=inicio,
                                   fecha__lte=fin).order_by('fecha').values("id", "fecha", "valor")
    ...
    caudal_mm = Decimal(caudal_promedio_serie) * 24 * 3600 * 365 / (area_influencia * 1000000)
    tablac = tablac.rename(columns=dict_mes, inplace=False)
    data_caudal = {
    ...
    for ep in est_precipitacion:
        res = mes.Var1Mensual.objects.filter(estacion_id__exact=ep.est_id, fecha__gte=inicio,
                                       fecha__lte=fin).order_by('fecha').values("id", "fecha", "valor")
        res = pd.DataFrame.from_records(res)
        tablap = tabla.copy()
        for e in list(range(0, len(res))):
