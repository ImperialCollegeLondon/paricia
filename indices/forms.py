...
from estacion.models import Estacion, Cuenca, SitioCuenca, Tipo
...
class EscorrentiaForm(forms.Form):
    sitiocuenca = forms.ModelChoiceField(queryset=SitioCuenca.objects.order_by('sitio_id'), label="Sitio - Cuenca")
    est_caudal = forms.ModelChoiceField(queryset=Estacion.objects.all(), widget=forms.RadioSelect, label="Estación caudal")
    est_precipitacion = forms.ModelMultipleChoiceField(queryset=Estacion.objects.all(), widget=forms.CheckboxSelectMultiple, label="Estación precipitación")
    inicio = forms.DateField(
        input_formats=['%Y-%m'], label="Inicio", required=True,
        widget=forms.TextInput(attrs={'autocomplete': 'off'})
    )
    fin = forms.DateField(
        input_formats=['%Y-%m'], label="Fin", required=True,
        widget=forms.TextInput(attrs={'autocomplete': 'off'})
    )
