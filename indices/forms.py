...
class SearchForm(forms.Form):
    ...
    estacion1 = forms.ModelChoiceField(
        queryset=Estacion.objects.order_by("est_id").all(),
        label="Primera Estaciòn",
        widget=forms.Select(attrs={"onChange": "cambio()"}),
    )
    estacion2 = forms.ModelChoiceField(
        queryset=Estacion.objects.order_by("est_id").all(),
        label="Segunda Estaciòn",
        widget=forms.Select(attrs={"onChange": "cambio()"}),
    )
...
class SelecEstForm(forms.Form):
    ...
    estacion = forms.ModelChoiceField(
        queryset=Estacion.objects.filter(tipo_id__in=[1, 2]).order_by("est_id"),
...
class IndCaudForm(forms.Form):
    estacion = forms.ModelChoiceField(
        queryset=Estacion.objects.filter(tipo_id__exact=3).order_by("est_id"),
        empty_label="Estación",
    )
    inicio = forms.DateField(
        widget=forms.TextInput(attrs={"autocomplete": "off"}),
...
class CuvarCaudalMultiestacionForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        estaciones = Estacion.objects.filter(
            tipo_id__exact=3, influencia_km__gt=0
        ).order_by("est_codigo")
        lista = []
        for e in estaciones:
            lista.append([e.est_id, e.est_codigo])
...
today = date.today()
years = [tuple([a, a]) for a in range(2000, today.year + 1)]


class EscorrentiaForm(forms.Form):
    sitiocuenca = forms.ModelChoiceField(
        queryset=SitioCuenca.objects.order_by("sitio_id"), label="Sitio - Cuenca"
    )
    est_caudal = forms.ModelChoiceField(
        queryset=Estacion.objects.all(),
        widget=forms.RadioSelect,
        label="Estación caudal",
    )
    est_precipitacion = forms.ModelMultipleChoiceField(
        queryset=Estacion.objects.all(),
        widget=forms.CheckboxSelectMultiple,
        label="Estación precipitación",
    )
    inicio = forms.DateField(
        input_formats=["%Y-%m"],
        label="Inicio",
    ...
    def __init__(self, *args, **kwargs):
        super(EscorrentiaForm, self).__init__(*args, **kwargs)
        self.fields["est_caudal"].choices = []
        self.fields["est_precipitacion"].choices = []
