...
from .functions import getVarValidado, acumularDoble, intensidadDiracion, getCaudalFrec, indicaCaudal, \
    consultaPeriodos, getCaudalFrecMulti, IndicadoresPrecipitacion, \
    calcular_escorrentia, calcular_dias_sin_caudal
import pandas as pd
...
from validacion.models import Var1Validado, Var10Validado
from estacion.models import Estacion, Tipo, SitioCuenca
...
class DuracionCaudal(generic.FormView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if data is None:
            data = [{}]
        return HttpResponse(json.dumps(data, allow_nan=True, cls=DecimalEncoder), content_type='application/json')
    ...
    def post(self, request, *args, **kwargs):
        ...
        if data is None:
            ...
            for d in data['anios']:
                ...
                ws.cell(row=fila, column=colu).alignment = Alignment(horizontal='center')
                if data['aporte']:
                    ws.merge_cells(start_row=fila, start_column=colu, end_row=fila, end_column=(colu+2))
                    ...
                    ws.merge_cells(start_row=fila, start_column=colu, end_row=fila, end_column=(colu + 1))
                    ws.cell(row=fila+1, column=colu).value = "Caudal"
                ...
                for v in range(0,len(data['anuales'][cu])):
                    if data['aporte']:
                        ws.cell(row=fila , column=colu-3).value = data['anuales'][cu][v]
                        ws.cell(row=fila , column=colu-2).value = data['anuales'][ce][v]
                        ws.cell(row=fila , column=colu-1).value = data['anuales'][fe][v]
                    else:
                        ws.cell(row=fila, column=colu-2).value = data['anuales'][cu][v]
                        ws.cell(row=fila, column=colu-1).value = data['anuales'][fe][v]
                    fila = fila + 1
...
def listar_anio_multi(request):
    ...
    data=[]
    if len(lisEsta) > 0:
        validados = Var1Validado.objects.filter(estacion_id__exact=lisEsta[0]).aggregate(Max('fecha'), Min('fecha'))
        if validados['fecha__max'] != None:
            fecha_min = validados['fecha__min'].year
            fecha_max = validados['fecha__max'].year
            for es in lisEsta:
                validados = Var1Validado.objects.filter(estacion_id__exact=es).aggregate(Max('fecha'), Min('fecha'))
                if validados['fecha__max'] != None:
...
def listar_fecha_caudal(request, estacion):
    data = Var10Validado.objects.filter(estacion_id__exact=estacion).aggregate(Max('fecha'), Min('fecha'))
    if data['fecha__max'] != None:
        #data['fecha__max'] = str(data['fecha__max'].day) +'/'+str(data['fecha__max'].month) +'/'+ str(data['fecha__max'].year)
        data['fecha__max'] = str(data['fecha__max'].year) +'-'+str(data['fecha__max'].month) +'-'+ str(data['fecha__max'].day)
        #data['fecha__min'] = str(data['fecha__min'].day) +'/'+str(data['fecha__min'].month) +'/'+ str(data['fecha__min'].year)
        data['fecha__min'] = str(data['fecha__min'].year) +'-'+str(data['fecha__min'].month) +'-'+ str(data['fecha__min'].day)
...
def listar_fecha_precipitacion(request, estacion):
    data = HorarioPrecipitacion.objects.filter(estacion_id__exact=estacion).aggregate(Max('fecha'), Min('fecha'))
    if data['fecha__max'] != None:
        #data['fecha__max'] = str(data['fecha__max'].day) +'/'+str(data['fecha__max'].month) +'/'+ str(data['fecha__max'].year)
        data['fecha__max'] = str(data['fecha__max'].year) +'-'+str(data['fecha__max'].month) +'-'+ str(data['fecha__max'].day)
        #data['fecha__min'] = str(data['fecha__min'].day) +'/'+str(data['fecha__min'].month) +'/'+ str(data['fecha__min'].year)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.request.is_ajax():
            if form.is_valid():
                ...
                res = calcular_escorrentia(sitiocuenca, est_caudal, est_precipitacion, inicio, fin)
...
def estaciones_en_cuenca(request, sitiocuenca_id):
    ...
    tipo_precipitacion = Tipo.objects.filter(nombre__in=['Pluviométrica', 'Climatológica'])
    est_precipitacion = Estacion.objects.filter(tipo__in=tipo_precipitacion, sitiocuenca__in=sitiocuenca)
    ...
    est_caudal = Estacion.objects.filter(tipo__in=tipo_caudal, sitiocuenca__in=sitiocuenca)
    opciones_caudal = [[x.est_id, x.est_codigo] for x in est_caudal]

    lista = {
        "precipitacion": opciones_precipitacion,
        "caudal": opciones_caudal
    }
